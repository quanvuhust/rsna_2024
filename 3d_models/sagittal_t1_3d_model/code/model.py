...
class Model(nn.Module):
    def __init__(self, back_bone, n_head, device_id):
        ...
        self.lstm = nn.GRU(lstm_embed, lstm_embed, num_layers=1, dropout=drop, bidirectional=True, batch_first=True).to(device_id)
    ...
    def forward(self, x0):
        ...
        # features0 = features0.reshape(bs, n_slice_per_c, -1)
        features0 = self.global_pool0(features0)
        features0 = features0.view(bs, n_slice_per_c, -1)
        features, _ = self.lstm(features0)
...
if __name__ == '__main__':
    model = Model("convnext_tiny.in12k_ft_in1k", 10, "cuda:0").cuda()
    print('Number of parameters: {}'.format(
        sum([p.data.nelement() for p in model.parameters()])))
    x0 = torch.rand((8, 8, 224, 224, 3)).cuda()
