...
from losses.custom_loss import AnySevereSpinal
...
def load_pretrain(model, sagittal_weight_path):
    ...
    model.model0.load_state_dict(pretrained_dict1)
...
def build_criterion(default_configs, device_id):
    ...
    obj_criterion = nn.BCEWithLogitsLoss().to(device_id)
    volume_criterion = CrossEntropyLoss(weight=torch.Tensor([1, 2, 4]).to(device_id)).to(device_id)
    any_criterion = AnySevereSpinal().to(device_id)
...
def build_net(default_configs, col_names, device_id, fold):
    ...
    # model = load_pretrain(model, "/root/sagittal_t2_2d_model/label_coordinates_weights/exp_10/{}/auc_exp_10_pretrain_sagittal_t2_fold{}.pt".format(fold, fold))
...
def train_one_fold(fold, train_loader, test_loader, rank, num_tasks, col_names, axis):
    ...
    for epoch in range(start_epoch, default_configs["num_epoch"]):
        ...
        for series_stacks_ax, volume_labels, any_labels, frame_labels, img_paths, series_ids in train_loader:
            ddp_model.train()
            ...
            if torch.rand(1)[0] < 0.5 and default_configs["use_mixup"]:
                mix_series_stacks_ax, volume_target_a, volume_target_b, frame_target_a, frame_target_b, lam = mixup(series_stacks_ax, volume_labels, frame_labels, alpha=default_configs["mixup_alpha"])
                ...
                with torch.cuda.amp.autocast():
                    volume_logits_3, image_logits = ddp_model(mix_series_stacks_ax)
                    loss_volume_3 = 0
                    ...
                    # any_loss = lam*any_criterion(volume_logits_3.permute(1, 2, 0), volume_target_a) + \
                    #     + (1-lam)*any_criterion(volume_logits_3.permute(1, 2, 0), volume_target_b)
                    # print("Mixup: ", any_loss)
                    # loss_volume_3 = (loss_volume_3 + any_loss)/2
                    # loss_volume_0 = 0
                ...
                with torch.cuda.amp.autocast():
                    volume_logits_3, image_logits = ddp_model(series_stacks_ax)
                    # loss_volume_0 = 0
                    # for i_head in range(volume_logits_0.shape[0]):
                    ...
                    loss_volume_3 = loss_volume_3/volume_logits_3.shape[0]
                    # any_loss = any_criterion(volume_logits_3.permute(1, 2, 0), volume_labels)
                    
                    loss_volume_3 = loss_volume_3
                    loss_image = 0; loss_obj = 0; loss_coord = 0
                    for i_head in range(image_logits.shape[0]):
