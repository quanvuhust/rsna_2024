...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, df, col_names, dataset, default_configs, mode):
        super().__init__()
        self.data_path = "/root/images/Sagittal T2"
        self.axial_crop_data_path = "/root/images/Sagittal T2"
        
        self.study_set = {}
        self.dataset = dataset
        cond_list = ["Spinal Canal Stenosis"]
        ...
        for index, row in self.label_coordinates_df.iterrows():
            ...
            # if study_id == 3637444890:
            #     print("HEHEHEHE")
            if condition != "Spinal Canal Stenosis":
                continue
            # if study_id == 3637444890:
            #     print("HIHIHIHI: ", series_id)
            if study_id in self.study_set.keys() and series_id in self.study_set[study_id].keys():
                ...
                if instance_number not in self.label_coordinates[study_id][series_id].keys():
                    self.label_coordinates[study_id][series_id][instance_number] = np.zeros((5,))
        ...
        for study_id in self.study_set.keys():
            ...
            for series_id in self.study_set[study_id].keys():
                ...
                for i in range(label.shape[0]):
                    ...
                    if study_id not in self.label_levels.keys() or condition_level not in self.label_levels[study_id][series_id].keys():
                        if type(label[i]) == str:
    ...
    def __getitem__(self, index):
        ...
        multiview_frame_labels = np.zeros((self.N_EVAL, 5))
        ...
        flag = False
        frame_labels = np.zeros((n_eval, 5))
        ...
        if study_id in self.label_coordinates.keys():
            if series_id in self.label_coordinates[study_id].keys():
                ...
                # num_select = min(n_eval, random.randint(1, total_instance))
                if self.mode == "train":
                    # if axis == "axial_t2":
