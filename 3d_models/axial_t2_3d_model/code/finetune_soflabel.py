...
def load_pretrain(model, axial_weight_path, sagittal_weight_path):
    # pretrained_dict = torch.load(axial_weight_path)
    # print(pretrained_dict.keys())
    ...
    pretrained_dict = torch.load(sagittal_weight_path)
...
def build_criterion(default_configs, device_id):
    ...
    direction_criterion = nn.BCEWithLogitsLoss().to(device_id)
    level_criterion = nn.BCEWithLogitsLoss().to(device_id)
    volume_criterion = CrossEntropyLoss(weight=torch.Tensor([1, 2, 4]).to(device_id)).to(device_id)
    any_criterion = nn.NLLLoss().to(device_id)
    left_severity_criterion = nn.CrossEntropyLoss(weight=torch.Tensor([1, 2, 4]).to(device_id)).to(device_id)
    right_severity_criterion = nn.CrossEntropyLoss(weight=torch.Tensor([1, 2, 4]).to(device_id)).to(device_id)
    return coord_criterion, direction_criterion, level_criterion, volume_criterion, any_criterion, left_severity_criterion, right_severity_criterion
...
def build_net(default_configs, col_names, device_id, fold):
    ...
    # model = load_pretrain(model, "", "/root/sagittal_model/label_coordinates_weights/exp_8/{}/auc_exp_8_pretrain_sagittal_fold{}.pt".format(fold, fold))
...
def train_one_fold(fold, train_loader, test_loader, rank, num_tasks, col_names, axis):
    ...
    coord_criterion, direction_criterion, level_criterion, volume_criterion, any_criterion, left_severity_criterion, right_severity_criterion = build_criterion(default_configs, device_id)
    ...
    for epoch in range(start_epoch, default_configs["num_epoch"]):
        ...
        loss_volume_train = 0; loss_coord_train = 0; loss_level_train = 0; loss_any_train = 0
        for series_stacks_ax, metadata_stacks, volume_labels, any_labels, frame_labels, severity_labels, img_paths, series_ids in train_loader:
            ddp_model.train()
            ...
            # metadata_stacks = metadata_stacks.to(device_id).float()
            # heat_map_labels = heat_map_labels.to(device_id).float()
            
            any_labels = any_labels.to(device_id).long()
            volume_labels = volume_labels.to(device_id).float()
            # print("HEHE: ", series_stacks.shape, frame_labels.shape, volume_labels.shape)
            if torch.rand(1)[0] < 0.5 and default_configs["use_mixup"]:
                mix_series_stacks_ax, volume_target_a, volume_target_b, frame_target_a, frame_target_b, severity_target_a, severity_target_b, lam = mixup(series_stacks_ax, volume_labels, frame_labels, severity_labels, alpha=default_configs["mixup_alpha"])
                frame_target_a = frame_target_a.view(frame_target_a.shape[0] * frame_target_a.shape[1], frame_target_a.shape[2], frame_target_a.shape[3])
                frame_target_b = frame_target_b.view(frame_target_b.shape[0] * frame_target_b.shape[1], frame_target_b.shape[2], frame_target_b.shape[3])
                severity_target_a = severity_target_a.view(severity_target_a.shape[0] * severity_target_a.shape[1], severity_target_a.shape[2])
                severity_target_b = severity_target_b.view(severity_target_b.shape[0] * severity_target_b.shape[1], severity_target_b.shape[2])
                with torch.cuda.amp.autocast():
                    volume_logits_3, coord_logits, level_logits = ddp_model(mix_series_stacks_ax)
                    loss_volume_3 = 0
                    ...
                    loss_image = 0; loss_obj = 0
                    
                    # loss_coord = 0
                    # for i_head in range(coord_logits.shape[0]):
                    #     loss_coord += lam*torch.sum(frame_target_a[:,i_head,0]*coord_criterion(coord_logits[i_head], frame_target_a[:,i_head,1:]))/(torch.sum(frame_target_a[:,i_head,0])+1e-6) + \
                    #         (1-lam)*torch.sum(frame_target_b[:,i_head,0]*coord_criterion(coord_logits[i_head], frame_target_b[:,i_head,1:]))/(torch.sum(frame_target_b[:,i_head,0])+1e-6)
                    # loss_coord = loss_coord/coord_logits.shape[0]
                    # print("1: ", loss_coord)
                    loss_level = lam*level_criterion(level_logits, frame_target_a[:, :, 0]) + \
                            (1-lam)*level_criterion(level_logits, frame_target_b[:, :, 0])
                    # loss_level /= default_configs["n_eval"]
                    # print(severity_image_logits.shape, severity_target_a.shape)
                    # loss_severity_left = lam*left_severity_criterion(severity_left_logits, severity_target_a[:, 0]) + \
                    #     (1-lam)*left_severity_criterion(severity_left_logits, severity_target_b[:, 0])
                    # loss_severity_right = lam*right_severity_criterion(severity_right_logits, severity_target_a[:, 1]) + \
                    #     (1-lam)*right_severity_criterion(severity_right_logits, severity_target_b[:, 1])
                    # loss_severity = (loss_severity_left + loss_severity_right)/2    
                    # loss_obj = loss_obj/obj_image_logits.shape[0]
                    # loss_image = loss_obj
                    ...
                    loss = loss_volume_3 + loss_level
                    loss_volume_train += loss_volume_3.item()
                    loss_level_train += loss_level.item()
                    # loss_direction_train += loss_direction.item()
                    # loss_coord_train += loss_coord.item()
                ...
                if ((batch_idx + 1) % default_configs["accumulation_steps"] == 0) or ((batch_idx + 1) == len(train_loader)):
                    ...
                    # torch.nn.utils.clip_grad_norm_(ddp_model.parameters(), 100)
                    scaler.step(optimizer_model)
                    scaler.update()
                    optimizer_model.zero_grad()
                    model_ema.update(ddp_model, step=num_updates)
            else:
                frame_labels = frame_labels.view(frame_labels.shape[0] * frame_labels.shape[1], frame_labels.shape[2], frame_labels.shape[3])
                severity_labels = severity_labels.view(severity_labels.shape[0] * severity_labels.shape[1], severity_labels.shape[2])
                # heat_map_labels = heat_map_labels.view(heat_map_labels.shape[0] * heat_map_labels.shape[1], heat_map_labels.shape[2])
                with torch.cuda.amp.autocast():
                    volume_logits_3, coord_logits, level_logits = ddp_model(series_stacks_ax)
   
                    loss_volume_3 = 0
                    for i_head in range(volume_logits_3.shape[0]):
                        loss_volume_3 += volume_criterion(volume_logits_3[i_head], volume_labels[:,i_head])
                    loss_volume_3 = loss_volume_3/volume_logits_3.shape[0]
                    loss_image = 0; loss_obj = 0; loss_severity = 0
                    # for i_head in range(obj_image_logits.shape[0]):
                    # loss_direction = direction_criterion(direction_logits, frame_labels[:, :2])
                    # loss_coord = 0
                    # print(frame_labels[:, :, 0].shape, coord_criterion(coord_logits[i_head], frame_labels[:,i_head,1:]).shape)
                    # print(loss_volume_3)
                    # for i_head in range(coord_logits.shape[0]):
                    #     loss_coord += torch.sum(frame_labels[:, i_head, 0]*coord_criterion(coord_logits[i_head], frame_labels[:,i_head,1:]))/(torch.sum(frame_labels[:, :, 0])+1e-6)
                    # loss_coord = loss_coord/coord_logits.shape[0]
                    # print("2: ", frame_labels[:,i_head,1:])
                    loss_level = level_criterion(level_logits, frame_labels[:, :, 0])
                    # loss_level /= default_configs["n_eval"]
                    # print(severity_image_logits.shape, severity_labels.shape)
                    # loss_severity_left = left_severity_criterion(severity_left_logits, severity_labels[:, 0])
                    # loss_severity_right = right_severity_criterion(severity_right_logits, severity_labels[:, 1])
                    # loss_severity = (loss_severity_left + loss_severity_right)/2
                    # loss_obj = loss_obj/obj_image_logits.shape[0]
                    # loss_severity = loss_severity/obj_image_logits.shape[0]
                    # loss_coord = loss_coord/image_logits.shape[0]
                    loss_image = loss_obj + loss_level
        ...
        # print("Train direction loss: ", loss_direction_train/batch_idx)
        print("Train level loss: ", loss_level_train/batch_idx)
...
if __name__ == '__main__':
    ...
    for axis in axis_list:
        ...
        soft_df = pd.read_csv(default_configs["softlabel_path"])
        col_names = df.columns.tolist()[11:21]
        avg_score = {"loss": 0}
        for col_name in col_names:
        ...
        for fold in folds:
            print("FOLD: ", fold)
            train_df = soft_df[soft_df["fold"] != fold]
            print(soft_df.shape, train_df.shape)
