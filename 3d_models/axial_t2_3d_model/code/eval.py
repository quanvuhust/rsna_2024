...
def eval(val_loader, model, device, epoch, is_ema, default_configs, col_names):
    ...
    with torch.no_grad():
        for batch_idx, (series_stacks_ax, metadata_stacks, volume_labels, any_labels, frame_labels, severity_labels, patient_ids, series_ids) in enumerate(val_loader):   
            series_stacks_ax = series_stacks_ax.to(device).float()
            volume_labels = volume_labels.to(device).long()
            # metadata_stacks = metadata_stacks.to(device).float()

            volume_logits_3, direction_logits, level_logits = model(series_stacks_ax)
