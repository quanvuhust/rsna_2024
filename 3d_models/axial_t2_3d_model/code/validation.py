...
def build_net(default_configs, col_names, device_id, fold):
    ...
    load_old_weight(model, "weights/{}/axial_t2/{}/axial_t2_loss_{}_fold{}.pt".format(default_configs["a_name"], fold, default_configs["a_name"], fold))
    flatNet = []
    shapes = []
    for param in model.parameters():
        ...
        if len(curr_shape) == 2:
            ...
            param = param.cpu().data.numpy().reshape(curr_shape[0]*curr_shape[1]*curr_shape[2])
            flatNet.append(param)
        elif len(curr_shape) == 4:
            param = param.cpu().data.numpy().reshape(curr_shape[0]*curr_shape[1]*curr_shape[2]*curr_shape[3])
            flatNet.append(param)
        elif len(curr_shape) == 5:
            param = param.cpu().data.numpy().reshape(curr_shape[0]*curr_shape[1]*curr_shape[2]*curr_shape[3]*curr_shape[4])
            flatNet.append(param)
        else:
...
def val_one_fold(fold, test_loader, col_names, axis):
    ...
    with torch.no_grad():
        for series_stack, volume_labels, any_labels, frame_labels, patient_ids in tqdm(test_loader):   
            ...
            for i in range(volume_logits.shape[1]):
                ...
                for j, col_name in enumerate(col_names):
                    ...
                    if "l1_l2" in col_name:
                        pred_rows.append({"row_id": row_id, "normal_mild": prob[j][0].item(), "moderate": prob[j][1].item(), "severe": 0.4*prob[j][2].item()})
                    else:
...
if __name__ == '__main__':
    ...
    f.close()
    default_configs["finetune"] = False
