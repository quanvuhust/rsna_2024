...
from eval import eval_pretrain
import argparse
import gc
...
def build_net(default_configs, device_id, col_names):
...
def train_one_fold(fold, train_loader, test_loader, rank, num_tasks, col_names):
    ...
    model = build_net(default_configs, device_id, col_names)
    ...
    for epoch in range(start_epoch, default_configs["num_epoch"]):
        ...
        loss_coord_train = 0
        for images, obj_labels in tqdm(train_loader):
            ddp_model.train()
            ...
            if torch.rand(1)[0] < 0.5 and default_configs["use_mixup"]:
                mix_images, obj_labels_a, obj_labels_b, lam = mixup(images, obj_labels, alpha=default_configs["mixup_alpha"])
                
                with torch.cuda.amp.autocast():
                    logits = ddp_model(mix_images)
                    loss_obj = 0; loss_coord = 0
                    for i_head in range(logits.shape[0]):
                        loss_coord += lam*coord_criterion(logits[i_head][:,1:], obj_labels_a[:,i_head, 1:]) + \
                            (1-lam)*coord_criterion(logits[i_head][:, 1:], obj_labels_b[:,i_head, 1:])
                ...
                with torch.cuda.amp.autocast():
                    ...
                    loss = 0; loss_obj = 0; loss_coord = 0
                    for i_head in range(logits.shape[0]):
        ...
        if rank == 0:
            ...
            val_metric = eval_pretrain(test_loader, model_ema.module, device_id, epoch, True, default_configs, col_names, coord_criterion)
            end = time.time()
            ...
            for val_metric_type in val_metric.keys():
                ...
                if val_metric_type == "loss":
                    ...
                    if flag == True:
                        best_model_path = os.path.join(weight_path, '{}_{}_fold{}.pt'.format(val_metric_type, default_configs["a_name"], fold))
                        try:
                        ...
                        torch.save(model_ema.module.model.state_dict(), best_model_path)
...
if __name__ == '__main__':
    ...
    df = pd.read_csv("coords_pretrain.csv")
    avg_score = {"loss": 0}
    for fold in folds:
        print("FOLD: ", fold)
        ...
        train_data = PreTrainDataset(train_df, default_configs, "train")
        col_names = train_data.level_list
        test_data = PreTrainDataset(val_df, default_configs, "test")
    ...
    if rank == 0:
        with mlflow.start_run(
            ...
            mlflow.set_tag("mlflow.runName", "rsna_pretrain")
