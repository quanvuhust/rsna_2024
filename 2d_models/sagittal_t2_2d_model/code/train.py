...
class CoordLoss(nn.Module):
    def __init__(self, device_id):
        ...
        self.loss = FocalLossBCE().to(device_id)
        self.device_id = device_id

    def forward(self, heat_map, target):
        return self.loss(heat_map, target)

def build_criterion(default_configs, device_id):
    obj_criterion = FocalLossBCE().to(device_id)
    coord_criterion = CoordLoss(device_id).to(device_id)
    return obj_criterion, coord_criterion

def build_net(default_configs, device_id):
    model = Model(default_configs["backbone"], 5, device_id).to(device_id)
    # if default_configs["axis"] == "sagittal":
...
def train_one_fold(fold, train_loader, test_loader, rank, num_tasks, col_names):
    ...
    obj_criterion, coord_criterion = build_criterion(default_configs, device_id)
    ...
    best_metric_ema = {"auc": {"score": 0, "list": []}, "f1_score": {"score": 0, "list": []}, "eer": {"score": 10000, "list": []}}
    best_model_path = ""
    ...
    for epoch in range(start_epoch, default_configs["num_epoch"]):
        ...
        loss_obj_train = 0
        loss_coord_train = 0
        for images, heat_maps, obj_labels in tqdm(train_loader):
            ddp_model.train()
            images = images.to(device_id).float()
            heat_maps = heat_maps.to(device_id).float()
            obj_labels = obj_labels.to(device_id).float()
            if torch.rand(1)[0] < 0.5 and default_configs["use_mixup"]:
                mix_images, heat_maps_a, heat_maps_b, obj_labels_a, obj_labels_b, lam = mixup(images, heat_maps, obj_labels, alpha=default_configs["mixup_alpha"])
                
                with torch.cuda.amp.autocast():
                    logits = ddp_model(mix_images)
                    loss_obj = 0
                    for i_head in range(logits.shape[0]):
                        loss_obj += lam*obj_criterion(logits[i_head][:, 0], obj_labels_a[:,i_head,0]) + \
                            (1-lam)*obj_criterion(logits[i_head][:, 0], obj_labels_b[:,i_head, 0])
                    
                    loss_obj = loss_obj/logits.shape[0]

                    # loss_coord = lam*coord_criterion(heat_map_logits, heat_maps_a) + (1-lam)*coord_criterion(heat_map_logits, heat_maps_b)
                    loss = loss_obj
                ...
                with torch.cuda.amp.autocast():
                    ...
                    loss_obj = 0; loss_coord = 0
                    for i_head in range(logits.shape[0]):
                        loss_obj += obj_criterion(logits[i_head][:, 0], obj_labels[:,i_head, 0])
        ...
        if rank == 0:
            ...
            val_metric = eval(test_loader, model_ema.module, device_id, epoch, True, default_configs, col_names, coord_criterion)
            end = time.time()
            ...
            for val_metric_type in val_metric.keys():
                ...
                if val_metric_type == "auc":
                    ...
                    if flag == True:
                        best_model_path = os.path.join(weight_path, '{}_{}_{}_fold{}.pt'.format(val_metric_type, default_configs["a_name"], default_configs["axis"], fold))
                        best_pretrain_path = os.path.join(weight_path, '{}_{}_pretrain_{}_fold{}.pt'.format(val_metric_type, default_configs["a_name"], default_configs["axis"], fold))
                        try:
        ...
        with mlflow.start_run(
            ...
            for fold in folds:
                with mlflow.start_run(experiment_id=experiment_id,
                    ...
                    score = train_one_fold(fold, train_loader_list[fold], test_loader_list[fold], rank, num_tasks, col_names) 
