...
def val_one_fold(fold, test_loader, col_names, axis):
    device_id = "cuda:0"
    if fold == 5:
        model = build_net(default_configs, col_names, device_id, 2)
    else:
    ...
    with torch.no_grad():
        for series_stacks_ax, series_stacks_sat2, series_stacks_sat1, volume_labels, any_labels, frame_labels, patient_ids in tqdm(test_loader):   
            ...
            volume_logits, image_logits = model(series_stacks_ax, series_stacks_sat2, series_stacks_sat1)
            for i in range(volume_logits.shape[1]):
                prob = torch.nn.Softmax(dim=1)(volume_logits[:, i, :])
                study_id = patient_ids[i].item()
                pred_rows.append({"study_id": study_id})
...
if __name__ == '__main__':
    ...
    for axis in axis_list:
        ...
        df_null = pd.read_csv("data/train_null.csv")
